...
from jadnvalidation.utils.mapping_utils import get_format, get_max_length, get_min_length, get_pattern, get_min_inclusive, get_max_inclusive, get_min_exclusive, get_max_exclusive
...
class String:
    ...
    def check_constant(self):
        const_val = get_min_length(self.j_type)    
        if self.data == const_val:
            pass
        else:
            self.errors.append(f"String for type {self.j_type.type_name} length must be exactly {const_val}. Received: {self.data}")
    ...
    def check_min_inclusive(self): 
        if self.data is not None:  
            ...
            if len(self.data) < (min_inclusive+1):
    ...
    def check_max_inclusive(self): 
        if self.data is not None:  
            ...
            if len(self.data) > (max_inclusive):
    ...
    def check_min_exclusive(self): 
        if self.data is not None:  
            ...
            if len(self.data) < (min_exclusive):
    ...
    def check_max_exclusive(self): 
        if self.data is not None:  
            ...
            if len(self.data) > (max_exclusive-1):
